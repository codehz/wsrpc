...
template <typename io = Output> struct Frame {
  ...
  inline Frame(FrameType type)
      : type(type) {}

  inline Frame(FrameType type, Data<io> const &payload)
      : type(type)
      , payload(payload) {}
  inline Frame(FrameType type, uint64_t eaten, Data<io> const &payload)
      : type(type)
      , eaten(eaten)
      , payload(payload) {}
};
