...
public:
  CommonException() { err = errno; }
  char const *what() { return strerror(err); }
};
...
void process(int fd) {
  ...
  while (type == FrameType::INCOMPLETE_FRAME) {
    ...
    if (state == State::STATE_OPENING) {
      ...
      switch (type) {
      case FrameType::INCOMPLETE_FRAME:
        continue;
      ...
      case FrameType::PING_FRAME:
        safeSend(fd, makeFrame({FrameType::PONG_FRAME}));
        break;
...
int main() {
  ...
  check(setsockopt(server, SOL_SOCKET, SO_REUSEPORT, &val, sizeof(val)),
        "setsockopt");
  ...
  while (true) {
    ...
    auto client =
        check(accept(server, (sockaddr *)&remote, &sockaddrlen), "accept");
    std::cout << "connected: "
              << inet_ntop(AF_INET, &remote.sin_addr, buffer, 32) << std::endl;
    try {
      process(client);
